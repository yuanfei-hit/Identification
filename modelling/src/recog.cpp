...
{
	bool flag = true;
	if (tactile_0.empty() || stiffness.empty())
		flag = false;
	else
	{
		...
		{
			 //=======================debug===============================//
			for (int j = 0; j < visual.points.size(); j++)
			{
				float rate = 6.0/7.0;
				visual.points[j].x = rate * visual.points[j].x;
				visual.points[j].y = rate * visual.points[j].y;
			}
			 //===========================================================//
			...
			if ((stiffness_similarity_scores[0].second < 1.0))
			{
				for (int i = 0; i < slice_scores.size(); i ++)
				{
					...
					{
						if (slice_scores[i].second[0].second > slice_scores[i].second[j].second)
							flag = false;
					}
				}
			}
	...
	//	if (align.hasConverged ())
		{
			if (score < mini)
			{
				Eigen::Matrix4f temp_tf = align.getFinalTransformation ();
			//	if (temp_tf(2, 2) != -1)
				{
					mini = score;
				//	std::cout << "mini_score: " << mini << std::endl;
					transformation = temp_tf;
...
std::vector<std::pair<int, std::vector<std::pair<int, float> > > > Recog::getSliceScores(int model_id, PointCloudT& visual, PointCloudNT& tactile_0, PointCloudNT& tactile_1, PointCloudNT& tactile_2)
{
	...
	*target = *sum + *target;
	...
	std::pair<float, Eigen::Matrix4f> score_tranform_1 = getAlignScoreAndTF(source, target, 5.0);
	std::pair<float, Eigen::Matrix4f> score_tranform_2 = getAlignScoreAndTF(target, source, 5.0);
	std::vector<std::pair<int, std::vector<std::pair<int, float> > > > slice_scores;
	Eigen::Affine3f  transform;

	if ((score_tranform_1.second(2, 2) != -1) && (score_tranform_2.second(2, 2) != -1))
	{
		if (score_tranform_1.first <= score_tranform_2.first)
			transform.matrix() = score_tranform_1.second;
		else
			transform.matrix() = score_tranform_2.second.inverse();
	}
	else
	{
		if (score_tranform_1.second(2, 2) != -1)
			transform.matrix() = score_tranform_1.second;
		else if (score_tranform_2.second(2, 2) != -1)
			transform.matrix() = score_tranform_2.second.inverse();
		else
			return slice_scores;
	}
	...
	for (int i = 0; i < this->database.slice_ids[d_index].size(); i ++)
	{
		...
		{
			int p_index = getClosestPointIndex(cloud_2, this->database.slice_heights[d_index][i]);
		...
		{
			int p_index = getClosestPointIndex(cloud_1, this->database.slice_heights[d_index][i]);
		...
		*fused_slice    = *visual_slice;
		...
		std::pair<float, Eigen::Matrix4f> score_tranform;
		score_tranform_1 = getAlignScoreAndTF(fused_slice, compaired_slice, 5.0);
		score_tranform_2 = getAlignScoreAndTF(compaired_slice, fused_slice, 5.0);
		if (score_tranform_1.first <= score_tranform_2.first)
			score_tranform = score_tranform_1;
		else
			score_tranform = score_tranform_2;
